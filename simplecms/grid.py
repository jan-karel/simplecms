...
class Grid:
    ...
    def __init__(self, SCMS, table, fields, q=False, menu=False, extra=False, well=False, path=False, edit=False, search=False, delete=False, new=False, view=False, smart=False):
        ...
        self.path = path if path else self.app.memory.folder +\
                              '/' + self.app.memory.appfolder + '/views/system'   
        self.well = well
        self.tot = False
    ...
    def query(self):
        #var limit set?
        if self.app.request.vars.page:
            a = (int(self.app.request.vars.page) * self.y) - self.y
            b = (int(self.app.request.vars.page) * self.y)
        else:
            a = (self.y - self.y)
            b = self.y
        if self.app.request.vars.search and self.cansearch:
            zoek = self.app.request.vars.search.replace('%20', ' ')
    ...
    def search_form(self):
        if not self.cansearch:
            return ''
        javascrip = "function submit_search()\
{if($('#gridsearchval').val()){location.replace('" + self.app.url + "?search=' + $('#gridsearchval').val());}}"

        self.app.javascript(code=javascrip)
        if self.smart:
            javascrip = "function smart_search()\
{alert(this);}"
            self.app.javascript(code=javascrip)
        ...
        self.html.input.add('', _type='search', name='search', _value='',
                                   _id='gridsearchval', _placeholder='search')
        self.html.div.add(self.html.input.xml(), _class='span8')
        if self.smart:
            self.html.button.add(self.app.T('query'), _onclick='smart_search()',
                                                                  _class='btn',_type='button')
        self.html.button.add(self.app.T('Zoeken', l='nl'), _onclick='submit_search()',
                                                                  _class='btn',_type='button')
    ...
    def menu(self, message=False, classname='error'):
        newl = self.app.T('{0}?edit=new&page={1}', [self.app.url,\
                                         self.app.request.vars.get('page', 1)])
        bl = self.app.T('{0}?page={1}', [self.app.url,\
                                         self.app.request.vars.get('page', 1)])

        btnnew = btn + ' icon-pencil btn-success pull-right'
        btnb = btn + ' icon-circle-arrow-left btn-warning pull-right'
        btnsearch = btn + ' icon-search pull-right t_function_show_gridsrchf '
        veld = ''
        if self.app.request.vars.show or self.app.request.vars.edit:
                    self.html.a.add(' ' + self.app.T('Terug', l='nl'), _href=bl,
                                                                   _class=btnb)
        if not self.app.request.vars.edit == 'new' and self.cannew:
            self.html.a.add(' ' + self.app.T('Nieuw',l='nl'), _href=newl, _class=btnnew)
        if self.app.request.vars.show and self.canedit:
            editl = self.app.T('{0}?edit={1}&page={2}', [self.app.url,\
             self.app.request.vars.show, self.app.request.vars.get('page', 1)])
            self.html.a.add(' ' + self.app.T('Bewerk'), _href=editl,
                                                                 _class=btnnew)
        if not self.app.request.vars.edit and self.cansearch:
    ...
    def show(self):
        if self.app.request.vars.edit:
            return self.edit(self.app.request.vars.edit)
        elif self.app.request.vars.show:
            return self.show_row()
        elif self.app.request.vars.xsearch:
            return self.show_results()
        elif self.app.request.vars.delete:
            del self.table[self.app.request.vars.delete]
    ...
    def show_results(self):
        res = self.app.db.smart_query([self.table],
                                         self.app.request.vars.search).select()
        return str(res)

    def show_row(self):
        #use template?
        rij = self.table[self.app.request.vars.show]
    ...
    def gridtable(self):
        ...
        javascrip = "function delete_item(e)\
{var item=m('del_'+e).getAttribute('data-del');m('delete_ok').href=item;$('#deleter').fadeIn();}"
        self.app.javascript(code=javascrip)
        #use template?
        ...
        if template == '404':
            ...
            if rijen:
                ...
                for x in rijen:
                    for g in self.fields:
                        ...
                        if not self.canedit:
                            ...
                            self.ppencil ='<a href="{0}?edit={1}&page={2}" class="icon-pencil btn smalltext"> </a>'
                        if not self.canview:
                            self.psearch = ''
                        else:
                            self.psearch = '<a href="{0}?show={1}&page={2}" class="icon-search btn smalltext"> </a>'
                        if not self.candelete:
                            self.pdelete = ''
                        else:
                            self.pdelete='<span id="del_{1}" onclick="delete_item(\'{1}\');" data-del="{0}?delete={1}&page={2}" class="icon-trash btn smalltext"> </span>'
                        if self.extra:
                            ex = self.extra
                    ...
                    self.html.td.add(self.app.T(d, [self.app.url,
                               x['id'], self.app.request.vars.get('page', 1)]))
                ...
                return self.message('Geen gegevens beschikbaar', classname='error icon-warning-sign')
        else:
            rijen = self.query()
            delen = (self.tot if self.tot else self.totaal()) / self.y + 1
    ...
    def pagination(self, delen):
        if delen > 1:
                loc = self.app.request.vars.get('page', 1)
                deelteller = 0
                ...
                """
                if int(loc) >= 3:
                    self.html.a.add(' <<', _href=self.app.url + '?page=1')
                ...
                for x in range(delen):
                    ...
                    href = self.app.url + '?page=' + str(x + 1)
                    if self.app.request.vars.search:
                        href = href + '&search=' + self.app.request.vars.search
    ...
    def edit(self, id=0):
        ...
        if self.app.post:
            #presave
            f.save(post=self.app.request.post)
            message = 'item saved'
            classname = 'success'

        #use template?
        path = self.path + '/grid/edit_' + str(self.table) + '.html'
        template = self.app.getfile(getfile=path)
        if template == '404':
            form = f.show() + '<button type="submit" class="btn btn-success bigger pull-right icon-ok"> '+self.app.T('Opslaan',l='nl')+'</button>'  
            self.html.form.add(form, _method="post", action=self.app.url)
